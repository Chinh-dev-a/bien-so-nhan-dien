...
def tachkytu(img):
    ...
    # cv2.imshow('morpho',morphology_img)
    _, thresh = cv2.threshold(equal_histogram, 125, 255, cv2.THRESH_BINARY_INV)
    # Tạo mask (bắt buộc phải lớn hơn ảnh 2 pixel mỗi chiều)
    cv2.imshow('chua su dung FloodFill',thresh)
    h, w = thresh.shape
    # print(h,w);
...
if __name__ == "__main__":
    img=cv2.imread("test/bienso/databienso/3_plate1.jpg")
    chars = tachkytu(img)
    print(len(chars))
    x=0
    while x!=len(chars):
